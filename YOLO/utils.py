...
def iou(yolo_pred_boxes, ground_truth_boxes):
    ...
    pred_boxes = tf.stack([pred_boxes[:, :, :, 0] - pred_boxes[:, :, :, 2]/2, pred_boxes[:, :, :, 1] - pred_boxes[:, :, :, 3]/2,
                          pred_boxes[:, :, :, 0] + pred_boxes[:, :, :, 2]/2, pred_boxes[:, :, :, 1] + pred_boxes[:, :, :, 3]/2])
    pred_boxes = tf.transpose(pred_boxes, [1, 2, 3, 0])

    true_boxes = ground_truth_boxes
    true_boxes = tf.stack([true_boxes[0] - true_boxes[2]/2, true_boxes[1] - true_boxes[3]/2,
                          true_boxes[0] + true_boxes[2]/2, true_boxes[1] + true_boxes[3]/2])
    ...
    mask = tf.cast(intersection[:, :, :, 0] > 0, tf.float32) * tf.cast(
        intersection[:, :, :, 1] > 0, tf.float32)  # 교집합이 존재하지 않는 경우 0

    inter_square = mask * intersection[:, :, :, 0] * intersection[:, :, :, 1]
    pred_box_square = (pred_boxes[:, :, :, 2] - pred_boxes[:, :, :, 0]) * \
        (pred_boxes[:, :, :, 3] - pred_boxes[:, :, :, 1])
    true_box_square = (true_boxes[2] - true_boxes[0]) * \
...
def find_max_confidence_bounding_box(bounding_box_info_list):
    bounding_box_info_list_sorted = sorted(
        bounding_box_info_list, key=itemgetter('confidence'), reverse=True)
