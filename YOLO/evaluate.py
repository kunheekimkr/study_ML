# Reference: https://github.com/solaris33/YOLO-v1-tf2/blob/main/evaluate.py
...
flags.DEFINE_string('test_dir', default='test_result',
...
voc2007_train_split_data = tfds.load(
...
def main(_):
    ...
    for image_num, features in enumerate(test_data):
        ...
        image, labels, object_num = process_each_ground_truth(
            batch_image[0], batch_bbox[0], batch_labels[0], input_width, input_height)
        ...
        output_image_name = os.path.join(
            os.getcwd(), FLAGS.test_dir, str(int(image_num)) + '_result.png')
        cv2.imwrite(output_image_name, cv2.cvtColor(
            drawing_image, cv2.COLOR_BGR2RGB))
